...
RSpec.describe Cart do
  describe 'Instance Methods' do
    ...
    it '.subtotal_of()' do
      ...
      expect(cart.subtotal_of(@ogre.id)).to eq(180.0)
      expect(cart.subtotal_of(@giant.id)).to eq(100)
    end

    it '.percent_of_price_with_discount()' do
      cart = Cart.new({
        @ogre.id.to_s => 10,
        @giant.id.to_s => 2
        })

      expect(cart.percent_of_price_with_discount(@ogre, cart.contents[@ogre.id.to_s])).to eq(0.9)
      expect(cart.percent_of_price_with_discount(@giant, cart.contents[@giant.id.to_s])).to eq(1.0)
    end
    ...
    it '.unit_price()' do
      ...
      expect(cart.unit_price(@ogre.id)).to eq(18.0)
      expect(cart.unit_price(@giant.id)).to eq(50.0)
    end
